...
namespace Utils 
{
    ...
    // Creation du "generateur aleatoire" necessaire à la generation de nombres aléatoires
    ...
    // ----------------------------------------
    // 2. Opérations sur vecteurs simples (exemple Vec3)
    // ----------------------------------------
    struct Vec3 
    {
        float x, y, z;
    ...
    // Lecture simple d’un fichier texte (retrourne un string complet)
    ...
    /// Ajoute (accrédite) à la rotation actuelle un incrément défini
    /// par trois angles Euler (en degrés) autour des axes X (pitch), Y (yaw), Z (roll).
    ...
    {
        // 1) Convertir l'incrément d'angles en radians
        float p = XMConvertToRadians(deltaPitchDeg);
        float y = XMConvertToRadians(deltaYawDeg);
        float r = XMConvertToRadians(deltaRollDeg);

        // 2) Construire le quaternion de l'incrément de rotation
        //    (équivalent à "tourner p autour de X, puis y autour de Y, puis r autour de Z")
        ...
        //     ce qui correspond à un repère local).
        XMVECTOR newQv = XMQuaternionMultiply(incQv, curQv);

        // 5) Normaliser (éviter la dérive numérique)
    ...
    // ----------------------------------------
    // 7. (Éventuel) Extensions futures…
    // ----------------------------------------
    // Vous pouvez ajouter ici : gestion des threads, alignement mémoire, etc.
}
